...
prophet_df = (df[['date','new_cases_rolling7','new_deaths_rolling7','full_vac_rolling7','resident_gg_rolling7']])
...
arima = "Arima"
arimax = "Arimax"
prophet_uni = "Univariate Prophet"
prophet_multi = "Multivariate Prophet"
...
if selected_series == arima:
    ...
    pred_df= pd.DataFrame(predictions,columns =['pred'], index=pd.date_range(start=testArima.index[-1]+timedelta(1), periods=periods_input, freq="D"))
...
if selected_series == arimax:
    ...
    exog = ['new_deaths_rolling7', 'full_vac_rolling7','resident_gg_rolling7']
    ...
    lower_ci = []
    history = [x for x in df['new_cases_rolling7']]
    history_exog = df[exog].values.tolist()
...
if selected_series == prophet_uni:
    ...
    fig = prophet_plot(trainfb,testfb,pred_df)
    st.write(fig)
    if st.checkbox('Show result in table'):
        st.markdown("Table: The cumulative number of new infections in the next 7 days")
        st.table(pred_df)
...
if selected_series == prophet_multi:
    ...
    future_add1 = model_add1.make_future_dataframe(periods=future_range+7,freq='D',include_history=False)
    forecast_add1 = model_add1.predict(future_add1)

    model_add2 = Prophet()
    model_add2.fit(add2_df)
    future_add2 = model_add2.make_future_dataframe(periods=future_range+7,freq='D',include_history=False)
    forecast_add2 = model_add2.predict(future_add2)

    model_add3 = Prophet()
    model_add3.fit(add3_df)
    future_add3 = model_add3.make_future_dataframe(periods=future_range+7,freq='D',include_history=False)
    ...
    fig = prophet_plot(trainProphet,testProphet,predict_future)
    st.write(fig)
    if st.checkbox('Show result in table'):
        st.markdown("Table: The cumulative number of new infections in the next 7 days")
